...
if !exists("c_no_cern_root")
   ...
   syn keyword	cType		TF1 TF2 TF3 TProfile2D TProfile
   syn keyword  cType   TMatrix
   syn keyword	cType		THStack TGraph TGraph2D TCanvas TRandom TLimit TLatex TGraphErrors TGraph2DErrors TMCParticle
   syn keyword	cType		TNtuple TObject TObjArray TClonesArray TTree TBranch TIter TKey TRandom2 TShape TPad TMacro
   syn keyword	cType           TString TLine TLegend TFile TDatime TRandom3 TMath TText TMultiGraph
   ...
   syn keyword	cType           Double_t UInt_t Int_t Float_t Long_t Bool_t Byte_t
   syn keyword	cType      	char_t Marker_t Double32_t Float16_t Long64_t
   syn keyword	cType           Short_t Stat_t Style_t Text_t UShort_t  
   syn keyword	cConstant       kBlack kGray kWhite kRed kPink kBlue kMagenta kViolet kAzure kCyan kTeal kGreen kSpring kYellow kOrange
